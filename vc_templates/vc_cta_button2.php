...
extract( shortcode_atts( array(
	...
	'accent_color' => '',
	'link' => '',
	'title' => __( 'Text on the button', "howes" ),
	'color' => 'skin',
	'icon' => '',
	'size' => 'md',
	'btn_style' => 'rounded',
...
?>
	<div<?php echo $inline_css; ?> class="<?php echo esc_attr( trim( $css_class ) ); ?>">
		<?php if ( $link != '' && $position != 'bottom' ) {
			echo do_shortcode( '[vc_button2 link="' . $link . '" title="' . $title . '" color="' . $color . '" icon="' . $icon . '" size="' . $size . '" style="' . $btn_style . '" el_class="vc_cta_btn" btnicon="'.$btnicon.'" btniconposition="'.$btniconposition.'"]' );
		} ?>
		<?php if ( $h2 != '' || $h4 != '' ): ?>
			<hgroup>
